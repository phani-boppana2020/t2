...
resource "google_project_iam_member" "project" {
  ...
  role = each.key
  member = "serviceAccount:terraform-sa1@plasma-range-343615.iam.gserviceaccount.com"
  project = var.project_id
...
#storage
resource "google_storage_bucket" "terraform-aloki2408" {
...
backend "gcs" {
  ...
  prefix = "firsttfstate" 
  depends_on = [google_storage_bucket.terraform-aloki2408]          
  }
